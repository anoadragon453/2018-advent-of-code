...
fn problem_one() -> Result<()> {
  ...
  for line in BufReader::new(file).lines() {
    ...
    for c in line.unwrap().chars() {
      // Get the current count of the letter if available
      let mut count = 0;
      ...
      // Record this letter
      letter_count.insert(c, count);
    }

    // Check if the count of any letter is 2/3
    for letter in letter_count.keys() {
      if letter_count[letter] == 2 {
        twice = 1;
      } else if letter_count[letter] == 3 {
        thrice = 1;
      }
    }

    // Possibly increase totals by 1
    total_twice += twice;
    total_thrice += thrice;
  ...
  println!("{}", total_twice * total_thrice);
  Ok(())
}
...
fn compute_distance(word_one: &String, word_two: &String) -> u32 {
  let mut distance = 0;

  for i in 0..word_one.chars().count() {
    if word_one.chars().nth(i) != word_two.chars().nth(i) {
      distance += 1;
    }
  }
...
fn common_letters(word_one: &String, word_two: &String) -> String {
  let mut common = String::new();
  for i in 0..word_one.chars().count() {
    if word_one.chars().nth(i) == word_two.chars().nth(i) {
      common.push(word_one.chars().nth(i).unwrap());
    }
  }
...
fn problem_two() -> Result<()> {
  // I need to learn how the borrow checker works :(
  let file = File::open("src/input")?;
  let lines: Vec<String> = BufReader::new(file).lines().map(|x| x.unwrap()).collect();

  for word_one_idx in 0..lines.len() {
    for word_two_idx in 0..lines.len() {
      if compute_distance(&lines[word_one_idx], &lines[word_two_idx]) == 1 {
        println!("{}", common_letters(&lines[word_one_idx], &lines[word_two_idx]));
        return Ok(())
      }
    }
  }
